// The total number of tweets from Colombia
...
connector
  ...
  .connect((connectError, session) => {
    ...
    session.query(query, defaultQueryOptions, (error, data) => {
      if (error) {
        return console.error('Query 1 error:', error)
      }
    ...
    session.query(query2, defaultQueryOptions, (error, data) => {
      if (error) {
        return console.error('Query 2 error:', error)
      }
      createRowChart(data)
      return console.log(
        'Query 2 results:',
...
// inspired by http://bl.ocks.org/d3noob/8952219
